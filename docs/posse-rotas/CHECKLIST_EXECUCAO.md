...
| 2026-07-13 | Revisão documental pós-rollout | `f5d9a88` | inventário de emissores; `pytest`; testes PostgreSQL em banco descartável; Vitest; build/lint; npm/pip audit; `compileall`; `pip check`; `git diff --check`; Alembic heads/current/history; inspeção visual de PDF; reinício e probes HTTP locais/públicos | termo institucional com brasão e assinaturas publicado; 194 backend, 29 PostgreSQL, 35 focados e 28 frontend aprovados; build 974 módulos; head/current 0040; pip-audit reduzido a 1 residual sem correção; login/health 200, docs 404 e rotas protegidas 401 | Codex |
