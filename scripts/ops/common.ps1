...
function Get-FrotaPaths {
    ...
    $paths = [ordered]@{
        ...
        BackendProductionScript = Join-Path $root "scripts\run-backend-production.ps1"
        FrontendDevScript       = Join-Path $root "scripts\run-frontend-dev.ps1"
        FrontendProductionScript = Join-Path $root "scripts\run-frontend-production.ps1"
        PostgresScript          = Join-Path $root "scripts\start_local_postgres.ps1"
        BackupScript            = Join-Path $root "scripts\backup-local.ps1"
        BackupAutoScript        = Join-Path $root "scripts\run-local-backup-automatico.ps1"
        BackupAutoInstallScript = Join-Path $root "scripts\install-local-autostart.ps1"
        WatchdogScript          = Join-Path $root "scripts\run-local-watchdog.ps1"
        WatchdogInstallScript   = Join-Path $root "scripts\install-local-autostart.ps1"
