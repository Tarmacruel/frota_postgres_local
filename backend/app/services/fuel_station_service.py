...
class FuelStationService:
    ...
    async def create(self, data: FuelStationCreate, actor: User) -> FuelStation:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Posto já cadastrado") from exc
    ...
    async def create_user_link(self, fuel_station_id: UUID, data: FuelStationUserCreate, actor: User) -> dict:
        ...
        try:
            ...
            await self.db.rollback()
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Vínculo já cadastrado") from exc
