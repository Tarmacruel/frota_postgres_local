...
@router.get("/active", response_model=list[DriverOut], dependencies=[Depends(require_permission("drivers", "view"))])
...
@router.get("", response_model=DriverListResponse, dependencies=[Depends(require_permission("drivers", "view"))])
async def list_drivers(
    ...
    return await DriverService(db).list(page=page, limit=limit, search=search, active_only=active, organization_id=organization_id)


@router.get("/{driver_id}", response_model=DriverOut, dependencies=[Depends(require_permission("drivers", "view"))])
